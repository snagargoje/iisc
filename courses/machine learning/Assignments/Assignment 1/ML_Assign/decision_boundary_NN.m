...
% displaydata 		- takes value 1/0 : display/not display the training data along with the decision boundary
    ...
    for i = 1:size(testset, 1)    
        %i
        classifier_labels(i) = kNN(traindata,trainlabels, k,[testset(i,1) testset(i,2)]);
    end
    toc
    ...
    plot(traindata(find(trainlabels == 1), 1), traindata(find(trainlabels == 1), 2), '.k');
    plot(traindata(find(trainlabels == -1), 1), traindata(find(trainlabels == -1), 2), '.y');
end
